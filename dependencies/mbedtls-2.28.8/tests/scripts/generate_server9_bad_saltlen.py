...
"""
SIG_OPT = r"-sigopt rsa_padding_mode:pss -sigopt rsa_pss_saltlen:{saltlen} -sigopt rsa_mgf1_md:sha256"
...
def build_argparser(parser):
    ...
    parser.add_argument("--ca-name", type=str, required=True, help="Basename of CA files")
    parser.add_argument("--ca-password", type=str, required=True, help="CA key file password")
    parser.add_argument("--csr", type=str, required=True, help="CSR file for generating certificate")
    parser.add_argument("--openssl-extfile", type=str, required=True, help="X905 v3 extension config file")
    parser.add_argument("--anounce_saltlen", type=auto_int, required=True, help="Announced salt length")
    parser.add_argument("--actual_saltlen", type=auto_int, required=True, help="Actual salt length")
...
def generate(**kwargs):
    ...
    if ca_password:
        kwargs["ca_password"] = r'-passin "pass:{ca_password}"'.format(**kwargs)
    else:
        kwargs["ca_password"] = ""
    extfile = kwargs.get("openssl_extfile", "")
    if extfile:
        kwargs["openssl_extfile"] = "-extfile {openssl_extfile}".format(**kwargs)
    else:
    ...
    der_bytes = subprocess.check_output(cmd, input=target_certificate["tbs_certificate"].dump(), shell=True)
