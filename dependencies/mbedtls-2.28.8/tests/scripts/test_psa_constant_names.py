...
def gather_inputs(headers: Iterable[str], test_suites: Iterable[str], inputs_class=InputsForTest) -> PSAMacroEnumerator:
...
def run_c(
    type_word: str, expressions: Iterable[str], include_path: Optional[str] = None, keep_c: bool = False
) -> List[str]:
...
ALG_TRUNCATED_TO_SELF_RE = re.compile(
    r"PSA_ALG_AEAD_WITH_SHORTENED_TAG\(" r"PSA_ALG_(?:CCM|CHACHA20_POLY1305|GCM)" r", *16\)\Z"
)
...
def collect_values(
    inputs: InputsForTest, type_word: str, include_path: Optional[str] = None, keep_c: bool = False
    ...
    expressions = sorted(expr for expr in inputs.generate_expressions(names) if not is_simplifiable(expr))
    values = run_c(type_word, expressions, include_path=include_path, keep_c=keep_c)
...
class Tests:
    ...
    Error = namedtuple("Error", ["type", "expression", "value", "output"])
    ...
    def run_one(self, inputs: InputsForTest, type_word: str) -> None:
        ...
        take arguments.
        """
        expressions, values = collect_values(
            inputs, type_word, include_path=self.options.include, keep_c=self.options.keep_c
        )
        output_bytes = subprocess.check_output([self.options.program, type_word] + values)
        ...
        for expr, value, output in zip(expressions, values, outputs):
            ...
            if normalize(expr) != normalize(output):
                self.errors.append(self.Error(type=type_word, expression=expr, value=value, output=output))

    def run_all(self, inputs: InputsForTest) -> None:
        """Run psa_constant_names on all the gathered inputs."""
        for type_word in ["status", "algorithm", "ecc_curve", "dh_group", "key_type", "key_usage"]:
    ...
    def report(self, out: typing_util.Writable) -> None:
        ...
        for error in self.errors:
            out.write(
                'For {} "{}", got "{}" (value: {})\n'.format(error.type, error.expression, error.output, error.value)
            )
...
def main():
    ...
    parser.add_argument("--include", "-I", action="append", default=["include"], help="Directory for header files")
    parser.add_argument(
        "--keep-c", action="store_true", dest="keep_c", default=False, help="Keep the intermediate C file"
    )
    parser.add_argument(
        "--no-keep-c", action="store_false", dest="keep_c", help="Don't keep the intermediate C file (default)"
    )
    parser.add_argument("--program", default="programs/psa/psa_constant_names", help="Program to test")
    parser.add_argument("--show", action="store_true", help="Show tested values on stdout")
    parser.add_argument("--no-show", action="store_false", dest="show", help="Don't show tested values (default)")
