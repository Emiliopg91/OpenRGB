...
def test_case_for_key_type_not_supported(
    verb: str, key_type: str, bits: int, dependencies: List[str], *args: str, param_descr: str = ""
) -> test_case.TestCase:
    ...
    tc.set_description("PSA {} {} {}-bit {} supported".format(verb, short_key_type, bits, adverb))
...
class KeyTypeNotSupported:
    ...
    def test_cases_for_key_type_not_supported(
        ...
        if kt.name in self.ALWAYS_SUPPORTED:
            ...
            return
        import_dependencies = [("!" if param is None else "") + psa_information.psa_want_symbol(kt.name)]
        if kt.params is not None:
            import_dependencies += [
                ("!" if param == i else "") + psa_information.psa_want_symbol(sym) for i, sym in enumerate(kt.params)
            ]
    ...
    ECC_KEY_TYPES = ("PSA_KEY_TYPE_ECC_KEY_PAIR", "PSA_KEY_TYPE_ECC_PUBLIC_KEY")
    ...
    def test_cases_for_not_supported(self) -> Iterator[test_case.TestCase]:
        ...
        for curve_family in sorted(self.constructors.ecc_curves):
            for constr in self.ECC_KEY_TYPES:
                ...
                yield from self.test_cases_for_key_type_not_supported(kt, param_descr="type")
                yield from self.test_cases_for_key_type_not_supported(kt, 0, param_descr="curve")


def test_case_for_key_generation(
    key_type: str, bits: int, dependencies: List[str], *args: str, result: str = ""
) -> test_case.TestCase:
    """Return one test case exercising a key generation."""
    psa_information.hack_dependencies_not_implemented(dependencies)
    tc = test_case.TestCase()
    short_key_type = crypto_knowledge.short_expression(key_type)
    tc.set_description("PSA {} {}-bit".format(short_key_type, bits))
    tc.set_dependencies(dependencies)
...
class KeyGenerate:
    ...
    @staticmethod
    def test_cases_for_key_type_key_generation(kt: crypto_knowledge.KeyType) -> Iterator[test_case.TestCase]:
        ...
        if kt.params is not None:
            import_dependencies += [psa_information.psa_want_symbol(sym) for i, sym in enumerate(kt.params)]
        if kt.name.endswith("_PUBLIC_KEY"):
...
class OpFail:
    ...
    def __init__(self, info: psa_information.Information) -> None:
        self.constructors = info.constructors
        key_type_expressions = self.constructors.generate_expressions(sorted(self.constructors.key_types))
        self.key_types = [crypto_knowledge.KeyType(kt_expr) for kt_expr in key_type_expressions]
    ...
    def make_test_case(
        ...
        if reason == self.Reason.NOT_SUPPORTED:
            short_deps = [re.sub(r"PSA_WANT_ALG_", r"", dep) for dep in not_deps]
        ...
        tc.set_description(
            "PSA {} {}: {}{}".format(
                category.name.lower(), pretty_alg, pretty_reason, " with " + pretty_type if pretty_type else ""
            )
        )
        ...
        error = "NOT_SUPPORTED" if reason == self.Reason.NOT_SUPPORTED else "INVALID_ARGUMENT"
    ...
    def no_key_test_cases(
        ...
        if alg.can_do(category):
            ...
            for dep in psa_information.automatic_dependencies(alg.base_expression):
                yield self.make_test_case(alg, category, self.Reason.NOT_SUPPORTED, not_deps=frozenset([dep]))
        else:
    ...
    def one_key_test_cases(
        ...
        for kt in self.key_types:
            ...
            if key_is_compatible and alg.can_do(category):
                ...
                for dep in psa_information.automatic_dependencies(alg.base_expression):
                    yield self.make_test_case(
                        alg, category, self.Reason.NOT_SUPPORTED, kt=kt, not_deps=frozenset([dep])
                    )
                # Public key for a private-key operation
                if category.is_asymmetric() and kt.is_public():
                    yield self.make_test_case(alg, category, self.Reason.PUBLIC, kt=kt)
            elif key_is_compatible:
                # Compatible key, incompatible operation, supported algorithm
                yield self.make_test_case(alg, category, self.Reason.INVALID, kt=kt)
            elif alg.can_do(category):
                # Incompatible key, compatible operation, supported algorithm
                yield self.make_test_case(alg, category, self.Reason.INCOMPATIBLE, kt=kt)
            else:
...
class StorageKey(psa_storage.Key):
    ...
    def __init__(self, usage: Iterable[str], without_implicit_usage: Optional[bool] = False, **kwargs) -> None:
...
class StorageTestData(StorageKey):
    ...
    def __init__(self, description: str, expected_usage: Optional[List[str]] = None, **kwargs) -> None:
...
class StorageFormat:
    ...
    @classmethod
    def exercise_key_with_algorithm(cls, key_type: psa_storage.Expr, bits: int, alg: psa_storage.Expr) -> bool:
    ...
    def key_for_lifetime(
        ...
        short = lifetime
        short = re.sub(r"PSA_KEY_LIFETIME_FROM_PERSISTENCE_AND_LOCATION", r"", short)
    ...
    def key_for_usage_flags(
        self, usage_flags: List[str], short: Optional[str] = None, test_implicit_usage: Optional[bool] = True
    ) -> StorageTestData:
        """Construct a test key for the given key usage."""
        extra_desc = " without implication" if test_implicit_usage else ""
        description = "usage" + extra_desc + ": "
        key1 = StorageTestData(
            version=self.version,
            id=1,
            lifetime=0x00000001,
            type="PSA_KEY_TYPE_RAW_DATA",
    ...
    def generate_keys_for_usage_flags(self, **kwargs) -> Iterator[StorageTestData]:
        ...
        for flag1, flag2 in zip(known_flags, known_flags[1:] + [known_flags[0]]):
    ...
    def keys_for_type(
        ...
        for bits in kt.sizes_to_test():
            ...
            compatible_algorithms = [alg for alg in all_algorithms if kt.can_do(alg)]
    ...
    def all_keys_for_types(self) -> Iterator[StorageTestData]:
        ...
        all_algorithms = [
            crypto_knowledge.Algorithm(alg)
            for alg in self.constructors.generate_expressions(sorted(self.constructors.algorithms))
        ]
...
class StorageFormatV0(StorageFormat):
    ...
    def keys_for_implicit_usage(
        self, implyer_usage: str, alg: str, key_type: crypto_knowledge.KeyType
    ) -> StorageTestData:
        ...
        description = "implied by {}: {} {} {}-bit".format(usage_expression, alg_expression, key_type_expression, bits)
        key = StorageTestData(
            version=self.version,
            id=1,
            lifetime=0x00000001,
    ...
    def gather_key_types_for_sign_alg(self) -> Dict[str, List[str]]:
        ...
        keyword_translation = {"ECDSA": "ECC", "ED[0-9]*.*": "EDWARDS"}
        exclusive_keywords = {"EDWARDS": "ECC"}
        ...
        for alg in algorithms:
            ...
            for key_type in key_types:
                ...
                if key_type_keywords.isdisjoint(incompatible_key_type_keywords) and not key_type_keywords.isdisjoint(
                    alg_keywords
                ):
...
class PSATestGenerator(test_data_generation.TestGenerator):
    ...
    targets = {
        "test_suite_psa_crypto_generate_key.generated": lambda info: KeyGenerate(info).test_cases_for_key_generation(),
        "test_suite_psa_crypto_not_supported.generated": lambda info: KeyTypeNotSupported(
            info
        ).test_cases_for_not_supported(),
        "test_suite_psa_crypto_low_hash.generated": lambda info: crypto_data_tests.HashPSALowLevel(
            info
        ).all_test_cases(),
        "test_suite_psa_crypto_op_fail.generated": lambda info: OpFail(info).all_test_cases(),
        "test_suite_psa_crypto_storage_format.current": lambda info: StorageFormatForward(info, 0).all_test_cases(),
        "test_suite_psa_crypto_storage_format.v0": lambda info: StorageFormatV0(info).all_test_cases(),
