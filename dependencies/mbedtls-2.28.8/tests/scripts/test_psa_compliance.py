...
def main():
    ...
    try:
        ...
        proc = subprocess.Popen(["./psa-arch-tests-crypto"], bufsize=1, stdout=subprocess.PIPE, universal_newlines=True)

        test_re = re.compile("^TEST: (?P<test_num>[0-9]*)|" "^TEST RESULT: (?P<test_result>FAILED|PASSED)")
        test = -1
