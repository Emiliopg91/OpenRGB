...
REVERSE_DEPENDENCIES = {
    ...
    "MBEDTLS_ECDSA_C": ["MBEDTLS_KEY_EXCHANGE_ECDHE_ECDSA_ENABLED", "MBEDTLS_KEY_EXCHANGE_ECDH_ECDSA_ENABLED"],
    "MBEDTLS_ECP_C": [
        "MBEDTLS_ECDSA_C",
        "MBEDTLS_ECDH_C",
        "MBEDTLS_ECJPAKE_C",
    ...
    "MBEDTLS_SHA256_C": ["MBEDTLS_KEY_EXCHANGE_ECJPAKE_ENABLED", "MBEDTLS_ENTROPY_FORCE_SHA256"],
    "MBEDTLS_X509_RSASSA_PSS_SUPPORT": [],
}
...
class DomainData:
    ...
    def config_symbols_matching(self, regexp):
        ...
        return [symbol for symbol in self.all_config_symbols if re.match(regexp, symbol)]
    ...
    def __init__(self, options, conf):
        ...
        self.domains = {
            ...
            "cipher_id": ExclusiveDomain(cipher_info.base_symbols, build_and_test),
            "cipher_chaining": ExclusiveDomain(cipher_chaining_symbols, build_and_test),
            "cipher_padding": ExclusiveDomain(cipher_padding_symbols, build_and_test),
            # Elliptic curves. Run the test suites.
            "curves": ExclusiveDomain(curve_symbols, build_and_test),
            # Hash algorithms. Exclude exclusive domain of MD, RIPEMD, SHA1 (obsolete)
            "hashes": DualDomain(
                hash_symbols, build_and_test, exclude=r"MBEDTLS_(MD|RIPEMD|SHA1_)" "|!MBEDTLS_*_NO_SHA"
            ),
            # Key exchange types.
            "kex": ExclusiveDomain(key_exchange_symbols, build_and_test),
            "pkalgs": ComplementaryDomain(
                [
                    "MBEDTLS_ECDSA_C",
                    "MBEDTLS_ECP_C",
                    "MBEDTLS_PKCS1_V21",
...
def main():
    try:
        parser = argparse.ArgumentParser(
            ...
            description="Test Mbed TLS with a subset of algorithms.\n\n"
            "Example usage:\n"
        ...
        parser.add_argument(
            "-c", "--config", metavar="FILE", help="Configuration file to modify", default="include/mbedtls/config.h"
        )
        parser.add_argument(
            "-C", "--directory", metavar="DIR", help="Change to this directory before anything else", default="."
        )
        parser.add_argument(
            "-k",
            "--keep-going",
        ...
        parser.add_argument(
            "--list-jobs", help="List supported jobs and exit", action="append_const", dest="list", const="jobs"
        )
        parser.add_argument(
            "--list-domains",
        ...
        parser.add_argument(
            "tasks", metavar="TASKS", nargs="*", help="The domain(s) or job(s) to test (default: all).", default=True
        )
