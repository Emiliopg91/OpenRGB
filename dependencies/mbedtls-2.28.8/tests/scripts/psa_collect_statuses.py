...
def collect_status_logs(options):
    ...
    if not os.path.exists(options.log_file):
        ...
        with open(os.devnull, "w") as devnull:
            make_q_ret = subprocess.call(["make", "-q", "lib", "tests"], stdout=devnull, stderr=devnull)
        if make_q_ret != 0:
            subprocess.check_call(["make", "RECORD_PSA_STATUS_COVERAGE_LOG=1"], stdout=sys.stderr)
            rebuilt = True
...
def main():
    ...
    parser.add_argument("--clean-after", action="store_true", help='Run "make clean" after rebuilding')
    parser.add_argument(
        "--clean-before", action="store_true", help='Run "make clean" before regenerating the log file)'
    )
    parser.add_argument(
        "--log-file",
        metavar="FILE",
    ...
    parser.add_argument(
        ...
        help="Path to psa_constant_names (default: {})".format(DEFAULT_PSA_CONSTANT_NAMES),
    )
    parser.add_argument(
        "--use-existing-log", "-e", action="store_true", help="Don't regenerate the log file if it exists"
    )
