...
def output_file_name(directory, stem, extension):
    return os.path.join(directory, "{}{}.{}".format(OUTPUT_FILE_PREFIX, stem, extension))
...
def run_one(options, args, stem_prefix="", input_file=None):
    ...
    * config-xxx.status: exit code.
    ...
    cmd = [os.path.abspath(options.script), "-f", os.path.basename(data_filename)]
    with open(stdout_filename, "wb") as out:
        with open(stderr_filename, "wb") as err:
            status = subprocess.call(
                cmd + args, cwd=options.output_directory, stdin=subprocess.DEVNULL, stdout=out, stderr=err
            )
...
def main():
    ...
    parser.add_argument("-d", metavar="DIR", dest="output_directory", required=True, help="""Output directory.""")
    parser.add_argument(
        "-f",
        metavar="FILE",
        dest="input_file",
    ...
    parser.add_argument(
        "-p", metavar="PRESET,...", dest="presets", help="""Presets to test (default: guessed from --help)."""
    )
    parser.add_argument(
        "-s",
        metavar="FILE",
