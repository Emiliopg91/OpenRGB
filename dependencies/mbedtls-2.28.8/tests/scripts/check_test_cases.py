...
class TestDescriptionExplorer:
    ...
    * Individual test cases in ssl-opt.sh.
    ...
    def process_test_case(self, per_file_state, file_name, line_number, description):
        """Process a test case.
        ...
        with open(data_file_name, "rb") as data_file:
            for line_number, line in enumerate(data_file, 1):
                ...
                if not in_paragraph:
                    ...
                    self.process_test_case(descriptions, data_file_name, line_number, line)
                in_paragraph = True
    ...
    def collect_from_script(self, file_name):
        ...
        for idx, description in enumerate(listed.splitlines()):
            self.process_test_case(descriptions, file_name, idx, description.rstrip())
    ...
    def walk_all(self):
        ...
        for directory in test_directories:
            for data_file_name in glob.glob(os.path.join(directory, "suites", "*.data")):
...
class TestDescriptions(TestDescriptionExplorer):
    ...
    def process_test_case(self, _per_file_state, file_name, _line_number, description):
    ...
    * Check that there is no duplicated description inside of one test suite.
    ...
    def process_test_case(self, per_file_state, file_name, line_number, description):
        ...
        if description in seen:
            results.error(file_name, line_number, "Duplicate description (also line {})", seen[description])
            return
        if re.search(rb"[\t;]", description):
            results.error(
                file_name,
                line_number,
                "Forbidden character '{}' in description",
                re.search(rb"[\t;]", description).group(0).decode("ascii"),
            )
        if re.search(rb"[^ -~]", description):
            results.error(file_name, line_number, "Non-ASCII character in description")
        if len(description) > 66:
            results.warning(file_name, line_number, "Test description too long ({} > 66)", len(description))
        seen[description] = line_number


def main():
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--list-all", action="store_true", help="List all test cases, without doing checks")
    parser.add_argument("--quiet", "-q", action="store_true", help="Hide warnings")
    parser.add_argument(
        "--verbose", "-v", action="store_false", dest="quiet", help="Show warnings (default: on; undoes --quiet)"
    )
