...
class BufferParameter:
    ...
    def __init__(self, i: int, is_output: bool, buffer_name: str, size_name: str) -> None:
...
class PSAWrapperGenerator(c_wrapper_generator.Base):
    ...
    def _return_variable_name(self, function: c_wrapper_generator.FunctionInfo) -> str:
        """The name of the variable that will contain the return value."""
        if function.return_type == "psa_status_t":
            return "status"
        return super()._return_variable_name(function)

    _FUNCTION_GUARDS = c_wrapper_generator.Base._FUNCTION_GUARDS.copy()  # pylint: disable=protected-access
    ...
    @staticmethod
    def _detect_buffer_parameters(
        arguments: List[c_parsing_helper.ArgumentInfo], argument_names: List[str]
    ) -> Iterator[BufferParameter]:
        ...
        for i, t01 in pairs:
            if (t01[0] == "const uint8_t *" or t01[0] == "uint8_t *") and t01[1] == "size_t":
                yield BufferParameter(i, not t01[0].startswith("const "), argument_names[i], argument_names[i + 1])

    @staticmethod
    def _write_poison_buffer_parameter(out: typing_util.Writable, param: BufferParameter, poison: bool) -> None:
        ...
        """
        out.write(
            "    MBEDTLS_TEST_MEMORY_{}({}, {});\n".format(
                "POISON" if poison else "UNPOISON", param.buffer_name, param.size_name
            )
        )

    def _write_poison_buffer_parameters(
        self, out: typing_util.Writable, buffer_parameters: List[BufferParameter], poison: bool
    ) -> None:
    ...
    @staticmethod
    def _parameter_should_be_copied(function_name: str, _buffer_name: Optional[str]) -> bool:
    ...
    def _write_function_call(
        self, out: typing_util.Writable, function: c_wrapper_generator.FunctionInfo, argument_names: List[str]
    ) -> None:
        buffer_parameters = list(
            param
            for param in self._detect_buffer_parameters(function.arguments, argument_names)
            if self._parameter_should_be_copied(function.name, function.arguments[param.index].name)
        )
...
class PSALoggingWrapperGenerator(PSAWrapperGenerator, c_wrapper_generator.Logging):
    ...
    def _printf_parameters(self, typ: str, var: str) -> Tuple[str, List[str]]:
        ...
        if typ == "psa_key_attributes_t *":
            return (
                var + "={id=%u, lifetime=0x%08x, type=0x%08x, bits=%u, alg=%08x, usage=%08x}",
                [
...
def main() -> None:
    ...
    parser.add_argument("--log", help="Stream to log to (default: no logging code)")
    parser.add_argument(
        "--output-c",
        metavar="FILENAME",
        default=DEFAULT_C_OUTPUT_FILE_NAME,
        help=("Output .c file path (default: {}; skip .c output if empty)".format(DEFAULT_C_OUTPUT_FILE_NAME)),
    )
    parser.add_argument(
        "--output-h",
        metavar="FILENAME",
        default=DEFAULT_H_OUTPUT_FILE_NAME,
        help=("Output .h file path (default: {}; skip .h output if empty)".format(DEFAULT_H_OUTPUT_FILE_NAME)),
    )
