...
def dependencies_of_symbol(symbol):
    ...
    if symbol.startswith("PSA_ALG_CATEGORY_") or symbol.startswith("PSA_KEY_TYPE_CATEGORY_"):
...
def systematic_dependencies(file_name, function_name, arguments):
    ...
    if (
        function_name.endswith("_key_policy")
        and arguments[-1].startswith("PSA_ERROR_")
        and arguments[-1] != ("PSA_ERROR_NOT_PERMITTED")
    ):
        arguments[-2] = ""
    if function_name == "copy_fail" and arguments[-1].startswith("PSA_ERROR_"):
        arguments[-2] = ""
        arguments[-3] = ""
    ...
    if os.path.basename(file_name) == "test_suite_psa_crypto_persistent_key.data" and function_name in {
        "format_storage_data_check",
...
def updated_dependencies(file_name, function_name, arguments, dependencies):
    ...
    manual = [dep for dep in dependencies if not (is_systematic_dependency(dep) or is_classic_dependency(dep))]
...
def process_data_stanza(stanza, file_name, test_case_number):
    ...
    if len(content_matches) < 2:
        raise Exception("Not enough content lines in paragraph {} in {}".format(test_case_number, file_name))
    if len(content_matches) > 3:
        raise Exception("Too many content lines in paragraph {} in {}".format(test_case_number, file_name))
    ...
    if len(content_matches) == 2:
        ...
        if dependencies_leader != "depends_on:":
            raise Exception(
                'Next-to-last line does not start with "depends_on:"'
                " in paragraph {} in {}".format(test_case_number, file_name)
            )
    new_dependencies = updated_dependencies(file_name, function_name, arguments, old_dependencies)
    if new_dependencies:
        stanza = text_before + dependencies_leader + ":".join(new_dependencies) + text_after
    else:
...
def process_data_file(file_name, old_content):
    ...
    new_stanzas = [process_data_stanza(stanza, file_name, n) for n, stanza in enumerate(old_stanzas, start=1)]
    return "\n\n".join(new_stanzas)
...
def process_file(file_name):
    ...
    if file_name.endswith(".data"):
        ...
        raise Exception("File type not recognized: {}".format(file_name))
