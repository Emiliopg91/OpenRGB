...
def looks_like_mbedtls_root(path: str) -> bool:
    ...
    return all(os.path.isdir(os.path.join(path, subdir)) for subdir in ["include", "library", "programs", "tests"])
...
def chdir_to_root() -> None:
    ...
    source tree.
    """
    for d in [os.path.curdir, os.path.pardir, os.path.join(os.path.pardir, os.path.pardir)]:
...
def guess_mbedtls_root():
    ...
    for frame in inspect.stack():
        ...
        for d in [".", os.path.pardir] + [os.path.join(*([os.path.pardir] * i)) for i in range(2, 10)]:
