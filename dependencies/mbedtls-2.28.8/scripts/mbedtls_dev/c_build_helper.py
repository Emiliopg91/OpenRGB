...
def create_c_file(file_label):
    ...
    * ``file_label``: a string that will be included in the file name.
    ...
    """
    c_fd, c_name = tempfile.mkstemp(prefix="tmp-{}-".format(file_label), suffix=".c")
...
def generate_c_file(c_file, caller, header, main_generator):
    ...
    * ``main_generator``: a function called with ``c_file`` as its sole argument
    ...
    """
    c_file.write("/* Generated by {} */".format(caller))
    c_file.write(
        """
#include <stdio.h>
"""
    ...
    * ``keep_c``: if true, keep the temporary C file (presumably for debugging
    ...
    try:
        ...
        generate_c_file(
            ...
            caller,
            header,
            lambda c_file: generate_c_printf_expressions(c_file, cast_to, printf_format, expressions),
        )
        c_file.close()
