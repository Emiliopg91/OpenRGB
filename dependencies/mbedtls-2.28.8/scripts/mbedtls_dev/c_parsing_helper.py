...
def read_logical_lines(filename: str) -> Iterator[Tuple[int, str]]:
    ...
    content = re.sub(_C_COMMENT_RE, lambda m: re.sub(_NOT_NEWLINES_RE, "", m.group(0)), content)
...
_C_FUNCTION_DECLARATION_RE = re.compile(
    ...
    r"(?P<return_type>\w[\w\s*]*?)\s*" + r"\b(?P<name>\w+)" + r"\s*\((?P<arguments>.*)\)\s*;",
    re.A | re.S,
)


def read_function_declarations(functions: Dict[str, FunctionInfo], filename: str) -> None:
