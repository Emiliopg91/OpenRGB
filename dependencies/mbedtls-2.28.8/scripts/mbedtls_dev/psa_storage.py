...
class Key:
    ...
    @staticmethod
    def pack(fmt: str, *args: Union[int, Expr]) -> bytes:  # pylint: disable=used-before-assignment
        ...
        * Only integer-valued elements are supported.
        """
        return struct.pack(
            "<" + fmt, *[arg.value() if isinstance(arg, Expr) else arg for arg in args]  # little-endian, standard sizes
        )
    ...
    def bytes(self) -> bytes:
        ...
        if self.version == 0:
            attributes = self.pack("LHHLLL", self.lifetime, self.type, self.bits, self.usage, self.alg, self.alg2)
...
class TestKey(unittest.TestCase):
    ...
    def test_numerical(self):
        ...
        expected_hex = (
            "505341004b45590000000000010000000024800000030000000250050010c00410000000404142434445464748494a4b4c4d4e4f"
        )
    ...
    def test_defaults(self):
        key = Key(type=0x1001, bits=8, usage=0, alg=0, alg2=0, material=b"\x2a")
