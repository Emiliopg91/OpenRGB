...
class HashPSALowLevel:
    ...
    def __init__(self, info: psa_information.Information) -> None:
        ...
        all_algorithms = [
            crypto_knowledge.Algorithm(expr) for expr in info.constructors.generate_expressions(base_algorithms)
        ]
        self.algorithms = [
            alg
            for alg in all_algorithms
            if (not alg.is_wildcard and alg.can_do(crypto_knowledge.AlgorithmCategory.HASH))
        ]
    ...
    @staticmethod
    def one_test_case(
        alg: crypto_knowledge.Algorithm, function: str, note: str, arguments: List[str]
    ) -> test_case.TestCase:
        """Construct one test case involving a hash."""
        tc = test_case.TestCase()
        tc.set_description("{}{} {}".format(function, " " + note if note else "", alg.short_expression()))
        tc.set_dependencies(psa_low_level_dependencies(alg.expression))
        tc.set_function(function)
        tc.set_arguments([alg.expression] + ['"{}"'.format(arg) for arg in arguments])
        return tc

    def test_cases_for_hash(self, alg: crypto_knowledge.Algorithm) -> Iterator[test_case.TestCase]:
        ...
        long = (
            b"Hello, world. Here are 16 unprintable bytes: ["
        ...
        yield self.one_test_case(alg, "hash_valid_one_shot", "", [short.hex(), hash_short])
        for n in [0, 1, 64, len(long) - 1, len(long)]:
            yield self.one_test_case(
                alg,
                "hash_valid_multipart",
                "{} + {}".format(n, len(long) - n),
                [long[:n].hex(), calc(long[:n]), long[n:].hex(), hash_long],
            )
