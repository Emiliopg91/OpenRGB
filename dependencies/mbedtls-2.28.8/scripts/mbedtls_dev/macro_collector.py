...
class read_file_lines:
    ...
    def __exit__(self, exc_type, exc_value, exc_traceback) -> None:
        ...
        if exc_type is not None:
            raise ReadFileLineException(self.filename, self.line_number) from exc_value
...
class PSAMacroEnumerator:
    ...
    def distribute_arguments_without_duplicates(self, seen: Set[str], name: str) -> Iterator[str]:
    ...
    def generate_expressions(self, names: Iterable[str]) -> Iterator[str]:
        ...
        * ``macros.generate_expressions(macros.key_types)`` generates all
        ...
        return itertools.chain(*(self.distribute_arguments_without_duplicates(seen, name) for name in names))
...
class PSAMacroCollector(PSAMacroEnumerator):
    ...
    _define_directive_re = re.compile(r"\s*#\s*define\s+(\w+)" + r"(?:\s+|\((\w+)\)\s*)" + r"(.+)")
    ...
    def read_line(self, line):
        ...
        if self.is_internal_name(name):
            ...
            return
        elif (name.startswith("PSA_ERROR_") or name == "PSA_SUCCESS") and not parameter:
            ...
            if name in ["PSA_ALG_ECDSA_BASE", "PSA_ALG_RSA_PKCS1V15_SIGN_BASE"]:
...
class InputsForTest(PSAMacroEnumerator):
    ...
    _header_line_re = re.compile(r"#define +" + r"(PSA_((?:(?:DH|ECC|KEY)_)?[A-Z]+)_\w+)" + r"(?:\(([^\n()]*)\))?")
    ...
    def parse_header_line(self, line: str) -> None:
        ...
        if re.search(self._excluded_name_re, name) or name in self._excluded_names or self.is_internal_name(name):
            return
    ...
    def add_test_case_line(self, function: str, argument: str) -> None:
        ...
        if function.endswith("_algorithm"):
            ...
            if function == "key_agreement_algorithm" and argument.startswith("PSA_ALG_KEY_AGREEMENT("):
