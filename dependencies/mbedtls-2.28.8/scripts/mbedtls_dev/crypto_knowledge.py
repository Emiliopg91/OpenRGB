...
class KeyType:
    ...
    def __init__(self, name: str, params: Optional[Iterable[str]] = None) -> None:
        ...
        self.params = None if params is None else [param.strip() for param in params]
    ...
    def key_material(self, bits: int) -> bytes:
        ...
        if self.expression in ASYMMETRIC_KEY_DATA:
            if bits not in ASYMMETRIC_KEY_DATA[self.expression]:
                raise ValueError("No key data for {}-bit {}".format(bits, self.expression))
            return ASYMMETRIC_KEY_DATA[self.expression][bits]
        if bits % 8 != 0:
            raise ValueError("Non-integer number of bytes: {} bits for {}".format(bits, self.expression))
        length = bits // 8
    ...
    def can_do(self, alg: "Algorithm") -> bool:
        ...
        if self.head in BLOCK_CIPHERS and alg.head in frozenset.union(
            BLOCK_MAC_MODES, BLOCK_CIPHER_MODES, BLOCK_AEAD_MODES
        ):
            if alg.head in ["CMAC", "OFB"] and self.head in ["ARIA", "CAMELLIA"]:
                return False  # not implemented in Mbed TLS
            return True
        if self.head == "CHACHA20" and alg.head == "CHACHA20_POLY1305":
            return True
        if self.head in {"ARC4", "CHACHA20"} and alg.head == "STREAM_CIPHER":
            return True
        if self.head == "RSA" and alg.head.startswith("RSA_"):
            return True
        if alg.category == AlgorithmCategory.KEY_AGREEMENT and self.is_public():
        ...
        if self.head == "ECC":
            ...
            if alg.head == "ECDH" and eccc in {
                EllipticCurveCategory.SHORT_WEIERSTRASS,
                EllipticCurveCategory.MONTGOMERY,
            }:
                return True
            if alg.head == "ECDSA" and eccc == EllipticCurveCategory.SHORT_WEIERSTRASS:
                return True
            if alg.head in {"PURE_EDDSA", "EDDSA_PREHASH"} and eccc == EllipticCurveCategory.TWISTED_EDWARDS:
                return True
        return False
...
class AlgorithmCategory(enum.Enum):
    ...
    def is_asymmetric(self) -> bool:
        ...
        return self in {self.SIGN, self.ASYMMETRIC_ENCRYPTION, self.KEY_AGREEMENT}
...
class Algorithm:
    ...
    def can_do(self, category: AlgorithmCategory) -> bool:
        ...
        if category == self.category:
            return True
        if category == AlgorithmCategory.KEY_DERIVATION and self.is_key_agreement_with_derivation():
            return True
        return False
    ...
    def usage_flags(self, public: bool = False) -> List[str]:
        ...
        if self.category == AlgorithmCategory.HASH:
            ...
            flags = ["SIGN_HASH", "SIGN_MESSAGE", "VERIFY_HASH", "VERIFY_MESSAGE"]
        elif self.category == AlgorithmCategory.CIPHER or self.category == AlgorithmCategory.AEAD:
            ...
            if not public:
                flags += ["DECRYPT"]
        elif self.category == AlgorithmCategory.KEY_DERIVATION or self.category == AlgorithmCategory.KEY_AGREEMENT:
            flags = ["DERIVE"]
        else:
