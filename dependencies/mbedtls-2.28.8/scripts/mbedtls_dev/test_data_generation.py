...
class BaseTarget(metaclass=ABCMeta):
    ...
    def description(self) -> str:
        ...
        if self.show_test_count:
            return "{} #{} {}".format(self.test_name, self.count, self.case_description).strip()
        else:
...
class TestGenerator:
    ...
    def __init__(self, _options) -> None:
        ...
        self.targets.update(
            {subclass.target_basename: subclass.generate_tests for subclass in BaseTarget.__subclasses__()}
        )
    ...
    def write_test_data_file(self, basename: str, test_cases: Iterable[test_case.TestCase]) -> None:
...
def main(args, description: str, generator_class: Type[TestGenerator] = TestGenerator):
    ...
    parser.add_argument(
        "targets", nargs="*", metavar="TARGET", help='Target file to generate (default: all; "-": none)'
    )
    ...
    if options.targets:
        ...
        options.targets = [
            os.path.basename(re.sub(r"\.data\Z", r"", target)) for target in options.targets if target != "-"
        ]
    else:
