...
def main() -> None:
    ...
    parser.add_argument("--no-act", "-n", action="store_true", help="Don't act, just print what will be done")
    parser.add_argument(
        "--pip-install-option", action="append", dest="pip_install_options", help="Pass this option to pip install"
    )
    parser.add_argument(
        "--pip-option", action="append", dest="pip_general_options", help="Pass this general option to pip"
    )
    parser.add_argument(
        "--user",
        action="append_const",
        ...
        const="--user",
        help="Install to the Python user install directory" " (short for --pip-install-option --user)",
    )
    parser.add_argument(
        "files",
        nargs="*",
        metavar="FILE",
        help="Requirement files" " (default: {} in the script's directory)".format(DEFAULT_REQUIREMENTS_FILE),
    )
    options = parser.parse_args()
    if not options.files:
        options.files = [os.path.join(os.path.dirname(__file__), DEFAULT_REQUIREMENTS_FILE)]
    reqs = Requirements()
    for filename in options.files:
        reqs.add_file(filename)
    reqs.write(sys.stdout)
    if not options.no_act:
        reqs.install(pip_general_options=options.pip_general_options, pip_install_options=options.pip_install_options)
