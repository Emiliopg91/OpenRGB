...
class CaseBuilder(macro_collector.PSAMacroCollector):
    ...
    def _make_append_case(name):
        template = "case %(name)s: " 'append(&buffer, buffer_size, &required_size, "%(name)s", %(length)d); ' "break;"
        return template % {"name": name, "length": len(name)}

    @staticmethod
    def _make_bit_test(var, flag):
        return BIT_TEST_TEMPLATE % {"var": var, "flag": flag, "length": len(flag)}

    def _make_status_cases(self):
        return "\n    ".join(map(self._make_return_case, sorted(self.statuses)))

    def _make_ecc_curve_cases(self):
        return "\n    ".join(map(self._make_return_case, sorted(self.ecc_curves)))

    def _make_dh_group_cases(self):
        return "\n    ".join(map(self._make_return_case, sorted(self.dh_groups)))
    ...
    def _make_key_type_from_curve_code(builder, tester):
        return KEY_TYPE_FROM_CURVE_TEMPLATE % {"builder": builder, "builder_length": len(builder), "tester": tester}

    @staticmethod
    def _make_key_type_from_group_code(builder, tester):
        return KEY_TYPE_FROM_GROUP_TEMPLATE % {"builder": builder, "builder_length": len(builder), "tester": tester}
    ...
    def _make_hash_algorithm_cases(self):
        return "\n    ".join(map(self._make_return_case, sorted(self.hash_algorithms)))

    def _make_ka_algorithm_cases(self):
        return "\n    ".join(map(self._make_return_case, sorted(self.ka_algorithms)))

    def _make_algorithm_cases(self):
        return "\n    ".join(map(self._make_append_case, sorted(self.algorithms)))

    @staticmethod
    def _make_algorithm_from_hash_code(builder, tester):
        return ALGORITHM_FROM_HASH_TEMPLATE % {"builder": builder, "builder_length": len(builder), "tester": tester}
    ...
    def _make_key_usage_code(self):
        return "\n".join([self._make_bit_test("usage", bit) for bit in sorted(self.key_usage_flags)])
    ...
    def write_file(self, output_file):
        ...
        data["key_type_code"] = self._make_ecc_key_type_code() + self._make_dh_key_type_code()
