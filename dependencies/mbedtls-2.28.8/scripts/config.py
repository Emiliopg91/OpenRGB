...
def include_in_crypto(name):
    ...
    if name.startswith("MBEDTLS_X509_") or name.startswith("MBEDTLS_SSL_") or name.startswith("MBEDTLS_KEY_EXCHANGE_"):
        return False
    if name in [
...
class ConfigFile(Config):
    ...
    default_path = [
        _path_in_tree,
        os.path.join(os.path.dirname(__file__), os.pardir, _path_in_tree),
        os.path.join(os.path.dirname(os.path.abspath(os.path.dirname(__file__))), _path_in_tree),
    ]
    ...
    def __init__(self, filename=None):
        ...
        if filename is None:
            for candidate in self.default_path:
                ...
                raise Exception("Mbed TLS configuration file not found", self.default_path)
        super().__init__()
    ...
    _section_line_regexp = r"\s*/?\*+\s*[\\@]name\s+SECTION:\s*" + r"(?P<section>.*)[ */]*"
    _config_line_regexp = re.compile(r"|".join([_define_line_regexp, _section_line_regexp]))
    ...
    def _parse_line(self, line):
        ...
        if m is None:
            ...
            template = (
                name,
                m.group("indentation"),
                m.group("define") + name + m.group("arguments") + m.group("separator"),
            )
            self.settings[name] = Setting(active, name, value, self.current_section)
            return template
...
if __name__ == "__main__":
    ...
    def main():
        ...
        parser.add_argument("--write", "-w", metavar="FILE", help="""File to write to instead of the input file.""")
        ...
        add_adapter(
            "full_no_deprecated",
            no_deprecated_adapter(full_adapter),
        ...
        add_adapter(
            "full_no_platform",
            no_platform_adapter(full_adapter),
        ...
        add_adapter("crypto", crypto_adapter(None), """Only include crypto features. Exclude X.509 and TLS.""")
        add_adapter(
            "crypto_baremetal",
            crypto_adapter(baremetal_adapter),
            """Like baremetal, but with only crypto features,
                    excluding X.509 and TLS.""",
        )
        add_adapter(
            "crypto_full",
            crypto_adapter(full_adapter),
