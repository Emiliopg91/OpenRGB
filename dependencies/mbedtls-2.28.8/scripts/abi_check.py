...
class AbiChecker:
    ...
    def __init__(self, old_version, new_version, configuration):
        ...
        self.can_remove_report_dir = not (os.path.exists(self.report_dir) or self.keep_all_reports)
    ...
    def _get_clean_worktree_for_git_revision(self, version):
        ...
        if version.repository:
            self.log.debug(
                "Checking out git worktree for revision {} from {}".format(version.revision, version.repository)
            )
            ...
            self.log.debug("Checking out git worktree for revision {}".format(version.revision))
            worktree_rev = version.revision
        worktree_output = subprocess.check_output(
            [self.git_command, "worktree", "add", "--detach", git_worktree_path, worktree_rev],
            cwd=self.repo_path,
            stderr=subprocess.STDOUT,
        )
        ...
        version.commit = (
            subprocess.check_output(
                [self.git_command, "rev-parse", "HEAD"], cwd=git_worktree_path, stderr=subprocess.STDOUT
            )
            .decode("ascii")
            .rstrip()
        )
    ...
    def _update_git_submodules(self, git_worktree_path, version):
        ...
        if not (os.path.exists(os.path.join(git_worktree_path, "crypto")) and version.crypto_revision):
            return

        if version.crypto_repository:
            fetch_output = subprocess.check_output(
                [self.git_command, "fetch", version.crypto_repository, version.crypto_revision],
    ...
    def _build_shared_libraries(self, git_worktree_path, version):
        ...
        make_output = subprocess.check_output(
            [self.make_command, "lib"], env=my_environment, cwd=git_worktree_path, stderr=subprocess.STDOUT
        )
    ...
    def _get_abi_dumps_from_shared_libraries(self, version):
        ...
        for mbed_module, module_path in version.modules.items():
            output_path = os.path.join(
                self.report_dir, "{}-{}-{}.dump".format(mbed_module, version.revision, version.version)
            )
            abi_dump_command = [
                "abi-dumper",
                module_path,
            ...
            abi_dump_output = subprocess.check_output(abi_dump_command, stderr=subprocess.STDOUT)
            ...
            for line_number, line in enumerate(fd, 1):
                ...
                if not is_generated:
                    ...
                    if "read" not in function_name.split("_"):
                        continue
                metadata = SimpleNamespace(filename=filename, line_number=line_number, description=description)
    ...
    def _get_storage_format_tests(self, version, git_worktree_path):
        ...
        storage_data_files = set(glob.glob("tests/suites/test_suite_*storage_format*.data"))
        ...
        for test_file in sorted(storage_data_files):
            self._read_storage_tests(git_worktree_path, test_file, test_file in to_be_generated, version.storage_tests)

    def _cleanup_worktree(self, git_worktree_path):
        """Remove the specified git worktree."""
        shutil.rmtree(git_worktree_path)
        worktree_output = subprocess.check_output(
            [self.git_command, "worktree", "prune"], cwd=self.repo_path, stderr=subprocess.STDOUT
        )
        self.log.debug(worktree_output.decode("utf-8"))
    ...
    def _remove_extra_detail_from_report(self, report_root):
        for tag in ["test_info", "test_results", "problem_summary", "added_symbols", "affected"]:
    ...
    def _abi_compliance_command(self, mbed_module, output_path):
        ...
        if self.skip_file:
            abi_compliance_command += ["-skip-symbols", self.skip_file, "-skip-types", self.skip_file]
        if self.brief:
    ...
    def _is_library_compatible(self, mbed_module, compatibility_report):
        ...
        output_path = os.path.join(
            self.report_dir, "{}-{}-{}.html".format(mbed_module, self.old_version.revision, self.new_version.revision)
        )
        try:
            subprocess.check_output(self._abi_compliance_command(mbed_module, output_path), stderr=subprocess.STDOUT)
            ...
            if self.brief:
                ...
                compatibility_report.append(
                    "Compatibility issues found for {}, " "for details see {}".format(mbed_module, output_path)
                )
            return False
        compatibility_report.append("No compatibility issues for {}".format(mbed_module))
        ...
        for test_data in sorted(missing):
            ...
            compatibility_report.append(
                'Test case from {} line {} "{}" has disappeared: {}'.format(
                    metadata.filename, metadata.line_number, metadata.description, test_data
                )
            )
        compatibility_report.append(
            "FAIL: {}/{} storage format test cases have changed or disappeared.".format(len(missing), len(old_tests))
            if missing
            else "PASS: All {} storage format test cases are preserved.".format(len(old_tests))
        )
        compatibility_report.append(
            "Info: number of storage format tests cases: {} -> {}.".format(len(old_tests), len(new_tests))
        )
        return not missing
    ...
    def get_abi_compatibility_report(self):
        ...
        compatibility_report = [
            "Checking evolution from {} to {}".format(
                self._pretty_revision(self.old_version), self._pretty_revision(self.new_version)
            )
        ]
        compliance_return_code = 0

        if self.check_abi:
            shared_modules = list(set(self.old_version.modules.keys()) & set(self.new_version.modules.keys()))
        ...
        if self.check_storage_tests:
            if not self._is_storage_format_compatible(
                self.old_version.storage_tests, self.new_version.storage_tests, compatibility_report
            ):
                compliance_return_code = 1
...
def run_main():
    try:
        ...
        parser.add_argument("-or", "--old-repo", type=str, help="repository for old version.")
        parser.add_argument("-oc", "--old-crypto-rev", type=str, help="revision for old crypto submodule.")
        parser.add_argument("-ocr", "--old-crypto-repo", type=str, help="repository for old crypto submodule.")
        parser.add_argument(
            "-n",
            "--new-rev",
            type=str,
        ...
        parser.add_argument("-nr", "--new-repo", type=str, help="repository for new version.")
        parser.add_argument("-nc", "--new-crypto-rev", type=str, help="revision for new crypto version")
        parser.add_argument("-ncr", "--new-crypto-repo", type=str, help="repository for new crypto submodule.")
        parser.add_argument(
            "-s",
            "--skip-file",
            type=str,
        ...
        parser.add_argument(
            "--check-abi", action="store_true", default=True, help="Perform ABI comparison (default: yes)"
        )
        parser.add_argument("--no-check-abi", action="store_false", dest="check_abi")
        parser.add_argument(
            "--check-api", action="store_true", default=True, help="Perform API comparison (default: yes)"
        )
        parser.add_argument("--no-check-api", action="store_false", dest="check_api")
        parser.add_argument(
            "--check-storage", action="store_true", default=True, help="Perform storage tests comparison (default: yes)"
        )
