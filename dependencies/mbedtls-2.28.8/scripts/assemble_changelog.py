...
class InputFormatError(Exception):
    def __init__(self, filename, line_number, message, *args, **kwargs):
        message = "{}:{}: {}".format(filename, line_number, message.format(*args, **kwargs))
...
class TextChangelogFormat(ChangelogFormat):
    ...
    def split_categories(cls, version_body):
        ...
        return [
            ...
            for title_match, title_line, body, body_line in zip(title_matches, title_lines, bodies, body_lines)
        ]
...
class ChangeLog:
    ...
    def add_categories_from_text(self, filename, line_offset, text, allow_unknown_category):
        """Parse a version section or entry file."""
        try:
            categories = self.format.split_categories(text)
        except CategoryParseError as e:
            raise InputFormatError(filename, line_offset + e.line_offset, e.error_message)
        for category in categories:
            if not allow_unknown_category and category.name not in self.categories:
                raise InputFormatError(
                    filename, line_offset + category.title_line, 'Unknown category: "{}"', category.name
                )

            body_split = category.body.splitlines()

            for line_number, line in enumerate(body_split, 1):
                if not self._only_url_re.match(line) and len(line) > MAX_LINE_LENGTH:
                    long_url_msg = (
                        ". URL exceeding length limit must be alone in its line."
                        if self._has_url_re.match(line)
                        else ""
                    )
                    raise InputFormatError(
                        filename,
    ...
    def __init__(self, input_stream, changelog_format):
        ...
        (self.header, self.top_version_title, top_version_body, self.trailer) = self.format.extract_top_version(
            whole_file
        )
...
class EntryFileSortKey:
    ...
    * Uncommitted entry files are sorted according to their modification time.
    ...
    def creation_hash(filename):
        ...
        """
        hashes = subprocess.check_output(["git", "log", "--format=%H", "--follow", "--", filename])
    ...
    def list_merges(some_hash, target, *options):
        ...
        """
        text = subprocess.check_output(["git", "rev-list", "--merges", *options, "..".join([some_hash, target])])
    ...
    def merge_hash(cls, some_hash):
        ...
        first_parents = cls.list_merges(some_hash, target, "--first-parent", "--reverse")
...
def main():
    ...
    parser.add_argument(
        "--output", "-o", metavar="FILE", help="Output changelog file" " (default: overwrite the input)"
    )
    parser.add_argument(
        "--list-files-only",
        action="store_true",
